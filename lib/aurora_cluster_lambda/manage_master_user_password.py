...
def handler(event, context):
    ...
    if event["RequestType"] in ["Create"]:
        try:
            ...
            waiter.wait(
                DBClusterIdentifier=cluster_id, WaiterConfig={"Delay": 30, "MaxAttempts": 20}
            )
            ...
            # cluster state does not change immediately after the modification, so we need to wait for a while
            time.sleep(15)
            waiter = rds_client.get_waiter("db_cluster_available")
            waiter.wait(
                DBClusterIdentifier=cluster_id, WaiterConfig={"Delay": 30, "MaxAttempts": 20}
